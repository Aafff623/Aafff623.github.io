// 技术栈标签自适应
const techTagList = [
    "JavaSE/MySQL/JDBC","HTML/CSS/JavaScript","Bootstrap/Tailwind CSS","Git/Gitee",
    "Spring Boot","Vue","React","Docker","Linux","算法","LeetCode"
];
...
function renderTechGraph() {
    ...
    nodes.forEach(node => {
        ...
        div.innerHTML = `<div class="tech-node-title">${node.title}${node.checked ? ' <span style="font-size:1.2em;color:#fff;">&#10003;</span>' : ''}</div><div class="tech-node-desc">${node.desc}</div>`;
    ...
    const startDrag = (x,y)=>{
        ...
        scrollLeft = graph.scrollLeft; scrollTop = graph.scrollTop;
    ...
    const endDrag = ()=>{ isDragging = false; graph.style.cursor = 'grab'; };
    ...
    graph.addEventListener('touchstart', e => { const t=e.touches[0]; startDrag(t.pageX,t.pageY); }, {passive:true});
